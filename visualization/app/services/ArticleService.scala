...
class ArticleService @Inject()(val reactiveMongoApi: ReactiveMongoApi,
  ...
  def findById(id: String): Future[Option[Article]] = {
    collection.flatMap(_.find(Json.obj("id" -> id.toInt)).one[Article])
  }

  def insert(user: Article) = {
